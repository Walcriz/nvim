...
local function set_indentation(indentation, bufnr, silent)
  local config = require("indentation.config")
...
function M.guess_from_buffer(bufnr)
  ...
  v_print(1, "Guess Indent")
  v_print(1, "Lines using tabs:", tab_lines_count)
  v_print(1, "Lines using spaces:", space_lines_count)
  if space_lines_count ~= 0 then
    v_print(1, "Relative Indentation")
    for k, v in pairs(relative_spaces) do
      v_print(1, " ", k, "spaces:", v)
    end

    v_print(1, "Absolute Indentation")
    for k, v in pairs(spaces) do
      v_print(1, " ", k, "spaces:", v)
    end
  end
  v_print(1, "Lines loaded:", v_num_lines_loaded)
  v_print(1, "Lines iterated:", v_lines_iterated)
  ...
  elseif space_lines_count > 0 then
    ...
    for n_spaces, relative_count in pairs(relative_spaces) do
      ...
      v_print(1, " ", n_spaces, "score:", score)
  ...
  if context then
    ...
    if not config.override_editorconfig then
      ...
      if editorconfig and (editorconfig.indent_style or editorconfig.indent_size or editorconfig.tab_width) then
        v_print(1, "Excluded because of editorconfig settings.")
        return
      end
    end
    ...
    v_print(1, "File type:", filetype)
    v_print(1, "Buffer type:", buftype)

    if vim.tbl_contains(config.filetype_exclude, filetype) then
      v_print(1, "Excluded because of filetype.")
      return
    end

    if vim.tbl_contains(config.buftype_exclude, buftype) then
      v_print(1, "Excluded because of buftype.")
      return
    end
  end
